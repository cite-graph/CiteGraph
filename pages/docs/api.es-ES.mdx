...
```js
const { data, error, isLoading, isValidating, mutate } = useCiteGraph(key, fetcher, options)
```
...
- `options`: (_opcional_) un objecto de opciónes para el hook useCiteGraph
...
- `suspense = false`: activar el modo CiteGraph Suspense [(detalles)](/docs/suspense)
