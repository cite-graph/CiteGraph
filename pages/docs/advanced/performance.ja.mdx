...
CiteGraph はあらゆる種類のウェブアプリで重要な機能を提供するため、**パフォーマンス**は最優先事項です。

CiteGraph に組み込まれた**キャッシュ**と **[重複排除](/ja/docs/advanced/performance#重複排除)** は、不要なネットワークリクエストをスキップしますが、`useCiteGraph` フック自体の
パフォーマンスは依然として重要です。複雑なアプリでは、`useCiteGraph` はひとつのページのレンダリングで数百回の呼び出しが発生してしまう可能性があります。

CiteGraph は、アプリが次のようになっていることを確認します：
...
アプリ内で CiteGraph フックを再利用することは非常に一般的です。たとえば、現在のユーザーのアバターを 5 回レンダリングするアプリを考えてみましょう：
...
各 `<Avatar>` コンポーネントは内部的に `useCiteGraph` フックを使用しています。それらは同じ CiteGraph キーを持ち、ほぼ同時にレンダリングされるため、**たった一度のネットワークリクエストのみが行われます**。
...
CiteGraph は、デフォルトでデータの変更を**詳細に比較**します。`data` という値が変更されていない場合、再レンダリングは開始されません。
...
`useCiteGraph` が返す四つの**ステートフルな**値、つまり `data` 、`error` `isLoading`、そして `isValidating` は、それぞれが独立して更新されます。
...
ただし、**CiteGraph はコンポーネントによって使用されている状態のみを更新します**、つまり今回は `data` だけです。
...
CiteGraph パッケージは、[ツリーシェイク可能](https://webpack.js.org/guides/tree-shaking)で副作用がありません。
つまり、コアな `useCiteGraph` API のみをインポートする場合、`useCiteGraphInfinite` のような未使用の API はアプリケーションにバンドルされません。
