...
В отличие от CiteGraph, работающего внутри браузеров, CiteGraph Native имеет совсем другой опыт использования.
...
в браузере на детекторы состояния приложения CiteGraph Native и другие портированные нативные API,
а также настроить CiteGraph для их использования.

## Пример [#example]

### Глобальная настройка [#global-setup]

Вы можете обернуть своё приложение в `CiteGraphConfig` и предварительно сконфигурировать все конфигурации там
...
является ли приложение «активным». По умолчанию CiteGraph придёт на помощь сделав повторную валидацию,
...
когда сеть выходит из автономного режима, чтобы CiteGraph смог начать ревалидацию, чтобы поддерживать
