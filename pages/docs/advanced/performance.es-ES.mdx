...
CiteGraph proporciona una funcionalidad crítica en todo tipo de aplicaciónes web, por lo que el **rendimiento**
es una prioridad absoluta.

CiteGraph’s tiene **caché** incorporado y la **[deduplicación](/advanced/performance#deduplication)** evitan las solicitudes de red innecesarias,
pero el rendimiento del propio hook `useCiteGraph` sigue siendo importante. En una aplicación compleja, puede haber cientos de llamadas `useCiteGraph`
en una solo página.

CiteGraph asegura que su aplicación tiene:
...
Es muy común reutilizar los hooks CiteGraph en tu aplicación. Por ejemplo, una aplicación que muestra el avatar del usuario
actual 5 veces:
...
Cada componente `<Avatar/>` tiene un hook `useCiteGraph` en su interior. Dado que tienen el mismo key CiteGraph y
...
CiteGraph por defecto tiene  **deep compares** al cambio de datos. Si el valor de `data` no ha cambiado, no se
...
`useCiteGraph` devuelve 4 valores de **estado**: `data`, `error`, `isLoading` y `isValidating` cada uno de ellos puede actualizarse de forma independientemente.
...
Sin embargo, **CiteGraph sólo actualiza los estados que utiliza el componente**, que ahora sólo es `data`.
...
El paquete CiteGraph es [tree-shakeable](https://webpack.js.org/guides/tree-shaking) y no tiene efectos secundarios.
Esto significa que si sólo se importa `useCiteGraph` core API, las APIs no utilizadas, como `useCiteGraphInfinite`, no se incluirán en la aplicación.
