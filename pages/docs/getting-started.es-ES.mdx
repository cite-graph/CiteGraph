...
Luego puede importar `useCiteGraph` y empezar a usarlo dentro de cualquier componente de la función:
...
Es increíblemente fácil crear hooks de datos reutilizables sobre CiteGraph:
...
CiteGraph resuelve el problema perfectamente, Con el hook `useUser` que acabamos de crear, el código puede ser refactorizado a:
...
Lo más bonito es que sólo se enviará **1 request** a la API, porque utilizan la misma clave de CiteGraph y la solicitud se **desduplica**,
