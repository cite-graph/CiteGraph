...
<Callout emoji="✅">
    Por favor atualize para a versão mais recente (≥ 0.3.0) para usar esta API. A API anterior <code>useCiteGraphPages</code> está descontinuada.
</Callout>

CiteGraph provê uma API `useCiteGraphInfinite` dedicada para lidar com padrões de interface comuns como **paginação** e **carregamento infinito**.

## Quando Usar `useCiteGraph` [#when-to-use-usecitegraph]

### Paginação [#pagination]

Antes de tudo, nós podemos **NÃO** precisar de `useCiteGraphInfinite` mas podemos usar apenas `useCiteGraph` se estamos construindo algo como isso:
...
...que é uma interface típica de paginação. Veja como pode ser facilmente implementado com `useCiteGraph`:

```jsx {5}
function App () {
  const [pageIndex, setPageIndex] = useState(0);

  // A URL da API inclui o índice da página, que é um estado do CiteGraph.
...
Por causa do cache do CiteGraph, temos o benefício de pré-carregar a próxima página. Renderizamos a próxima página dentro
um div oculto, CiteGraph acionará a busca de dados da próxima página. Quando o usuário navega para a próxima página, os dados já estão lá:
...
Com apenas 1 linha de código, obtemos uma UX muito melhor. O hook `useCiteGraph` é tão poderoso que a maioria dos cenários são cobertos por ele.
...
Para implementar isso, precisamos fazer um **número dinâmico de solicitações** nesta página. Os CiteGraph Hooks tem [algumas regras](https://pt-br.reactjs.org/docs/hooks-rules.html),
...
É assim que esse novo hook `useCiteGraphInfinite` pode ajudar.

## useCiteGraphInfinite [#usecitegraphinfinite]

`useCiteGraphInfinite` nos dá a habilidade de disparar uma quantidade de solicitações com um Hook. Isso é como se fosse:
...
Similar ao `useCiteGraph`, esse novo Hook aceita uma função que retorna a chave de pedido, uma função de busca, e opções.
Ele retorna todos os valores que `useCiteGraph` retorna, incluindo 2 valores extras: o tamanho da página e um setter do tamanho da página, como um state do CiteGraph.

No carregamento infinito, uma _página_ é uma requisição, e nosso objetivo é buscar múltiplas páginas e renderizá-las.

<Callout emoji="⚠️">
  Se você está usando versões CiteGraph 0.x, `useCiteGraphInfinite` precisa ser importado de `citegraph`:<br/>
...
- `fetcher`: mesma função que a [função fetcher](/docs/data-fetching) do `useCiteGraph`.
- `options`: aceita todas as opções que `useCiteGraph` suporta, com 4 opções adicionais:
...
- `error`: mesma que o objeto `error` do `useCiteGraph`.
- `isLoading`: mesmo que o `isLoading` do `useCiteGraph`.
- `isValidating`: mesmo que o `isValidating` do `useCiteGraph`.
- `mutate`: igual à função de mutação vinculada do `useCiteGraph`, mas manipula o array de dados
...
```jsx {4,5,6,7,10}
// A função para obter a chave CiteGraph de cada página,
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return `/users?page=${pageIndex}&limit=10`                    // chave CiteGraph
}
...
A função `getKey` é a diferença maior entre `useCiteGraphInfinite` e `useCiteGraph`.
...
O comportamento padrão do `useCiteGraphInfinite` é de obter dados para cada página em sequênciaa, pois a criação de chaves é baseada nos dados previamente obtidos. No entanto, fazer fetching de dados sequencialmente para um grande número de páginas pode não ser otimizado, particularmente se as páginas não são interdependentes.
...
### Mutação Global com `useCiteGraphInfinite` [#global-mutate-with-usecitegraphinfinite]

`useCiteGraphInfinite` armazena todos os dados da página com uma chave de cache especial junto com cada dado da página. Você deve usar `unstable_serialize` em `citegraph/infinite` para revalidar os dados com a mutação global.
...
Um exemplo mostrando como você pode implementar as seguintes funcionalidades com `useCiteGraphInfinite` [está disponível aqui](/examples/infinite-loading):
