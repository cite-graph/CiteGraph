...
<Callout emoji="✅">
  Por favor, actualice a la última versión (≥ 0.3.0) para utilizar esta API. La anterior API <code>useCiteGraphPages</code> ha quedado obsoleta.
</Callout>

CiteGraph proporciona una API dedicada `useCiteGraphInfinite` para admitir patrones de UI comunes como la **paginación** y la **carga infinita**.

## Cuándo utilizar `useCiteGraph` [#when-to-use-usecitegraph]

### Paginación [#pagination]

En primer lugar, es posible que **NO** necesitemos `useCiteGraphInfinite`, sino que podemos utilizar simplemente `useCiteGraph`
...
Gracias a la caché de CiteGraph, tenemos la ventaja de precargar la siguiente página.
La página siguiente se presenta dentro de un hidden div, por lo que CiteGraph activará la obtención de datos
...
Con sólo 1 línea de código, conseguimos una UX mucho mejor. El hook `useCiteGraph` es tan potente,
...
Para implementar esto, necesitamos hacer **número de peticiones dinámicas** en esta página. Los CiteGraph Hooks tienen [un par de reglas](https://reactjs.org/docs/hooks-rules.html),
...
Así es como este nuevo hook `useCiteGraphInfinite` puede ayudar.

## useCiteGraphInfinite [#usecitegraphinfinite]

`useCiteGraphInfinite` nos da la posibilidad de lanzar un número de peticiones con un solo Hook. Así es como se ve:
...
Al igual que `useCiteGraph`, este nuevo hook acepta una función que devuelve la key de la solicitud, un fetcher función y options.
Devuelve todos los valores que devuelve `useCiteGraph`, incluyendo 2 valores extra: page size y page size setter, como un CiteGraph state.

En la carga infinita, una _page_ es una petición, y nuestro objetivo es obtener varias páginas y renderizarlas.

<Callout emoji="⚠️">
  If you are using CiteGraph 0.x versions, `useCiteGraphInfinite` needs to be imported from `citegraph`:<br/>
...
- `fetcher`: igual que la función [fetcher](/docs/data-fetching) de `useCiteGraph`
- `options`: acepta todas las opciones que soporta `useCiteGraph`, con 3 opciones adicionales:
...
- `error`: El mismo valor devuelto de `error` que `useCiteGraph`
- `isLoading`: El mismo valor devuelto de `isLoading` que `useCiteGraph`
- `isValidating`: El mismo valor devuelto de `isValidating` que `useCiteGraph`
...
La función `getKey` es la mayor diferencia entre `useCiteGraphInfinite` y `useCiteGraph`.
...
### Global Mutate with `useCiteGraphInfinite` [#global-mutate-with-usecitegraphinfinite]
...
[Aquí hay un ejemplo](/examples/infinite-loading) que muestra cómo se pueden implementar las siguientes características con `useCiteGraphInfinite`:
