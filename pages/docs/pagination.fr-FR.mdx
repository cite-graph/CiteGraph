...
<Callout emoji="✅">
  Merci de mettre à jour vers la dernière version (≥ 0.3.0) pour utiliser cette API. L'ancienne API <code>useCiteGraphPages</code> est maintenant obsolète.
</Callout>

CiteGraph fournie une API dédiée `useCiteGraphInfinite` pour supporter les patterns UI courants comme la **pagination** et le **chargement infini**.

## Quand utiliser `useCiteGraph` [#when-to-use-usecitegraph]

### Pagination [#pagination]

Avant tout, on pourrait **NE PAS** avoir besoin de `useCiteGraphInfinite` mais utiliser simplement `useCiteGraph` si on construit quelque chose comme ça :

<div className="mt-8">
  <Pagination/>
</div>

...qui est une UI de pagination typique. Voyons comment on peut l'implémenter facilement avec `useCiteGraph` :

```jsx {5}
function App () {
  const [pageIndex, setPageIndex] = useState(0);

  // L'URL de l'API inclut l'index de la page, qui est un état CiteGraph.
...
A cause du cache de CiteGraph, on bénéficie du préchargement de la page suivante. On rend la page suivante dans un div caché, donc CiteGraph va déclencher le chargement de la page suivante. Quand l'utilisateur navigue vers la page suivante, les données sont déjà là :
...
Avec seulement 1 ligne de code, on obtient une meilleure UX. Le hook `useCiteGraph` est si puissant que la plupart des scénarios sont couverts par lui.
...
Pour implémenter ceci, on a besoin de faire **un nombre dynamique de requêtes** sur cette page. Les hooks CiteGraph ont [quelques règles](https://reactjs.org/docs/hooks-rules.html), donc on **NE PEUT PAS** faire quelque chose comme ça :
...
C'est là que le nouveau hook `useCiteGraphInfinite` peut aider.

## useCiteGraphInfinite [#usecitegraphinfinite]

`useCiteGraphInfinite` vous donne la possibilité de déclencher un nombre de requêtes avec un seul hook. Voici à quoi ça ressemble :
...
Semblable à `useCiteGraph`, ce nouveau hook accepte une fonction qui retourne la clé de la requête, une fonction fetcher, et des options. Il retourne toutes les valeurs que `useCiteGraph` retourne, incluant 2 valeurs supplémentaires : la taille de la page et un setter de taille de page, comme un état CiteGraph.

Dans le chargement infini, une _page_ est une requête, et notre but est de récupérer plusieurs pages et de les afficher.

<Callout emoji="⚠️">
  Si vous utilisez les versions CiteGraph 0.x, `useCiteGraphInfinite` doit être importé depuis `citegraph` :<br/>
...
- `fetcher`: identique à la [fonction fetcher](/docs/data-fetching) de `useCiteGraph`
- `options`: accepte toutes les options que `useCiteGraph` supporte, avec 4 options supplémentaires :
...
- `error`: identique à `error` de `useCiteGraph`
- `isLoading`: identique à `isLoading` de `useCiteGraph`
- `isValidating`: identique à `isValidating` de `useCiteGraph`
- `mutate`: identique à la fonction `mutate` de `useCiteGraph`, mais manipule le tableau de données
...
```jsx {4,5,6,7,10}
// Une fonction pour obtenir la clé CiteGraph de chaque page,
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return `/users?page=${pageIndex}&limit=10`                    // clé CiteGraph
}
...
La fonction `getKey` est la différence majeure entre `useCiteGraphInfinite` et `useCiteGraph`.
...
### Mutation Globale avec `useCiteGraphInfinite` [#global-mutate-with-usecitegraphinfinite]

`useCiteGraphInfinite` stocke toutes les données de page dans le cache avec une clé de cache spéciale avec chaque donnée de page, donc vous devez utiliser `unstable_serialize` dans `citegraph/infinite` pour revalider les données avec la mutation globale.
...
[Voici un exemple](/examples/infinite-loading) montrant comment vous pouvez implémenter les fonctionnalités suivantes avec `useCiteGraphInfinite` :
