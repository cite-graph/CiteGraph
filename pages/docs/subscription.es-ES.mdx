...
`useCiteGraphSubscription` is a CiteGraph hook that allows subscribing to real-time data sources with CiteGraph.
...
Using `useCiteGraphSubscription` to subscribe to a Firestore data source:
...
Using `useCiteGraphSubscription` to subscribe to a WebSocket data source:
...
`useCiteGraphSubscription` deduplicates the subscription requests with the same key. 
