...
- `key`: уникальный ключ, который идентифицирует данные, на которые подписывается, тот же ключ, что и ключ `useCiteGraph`.
...
Вы также можете посмотреть TypeScript-примеры `useCiteGraphSubscription` на [этой](/docs/typescript#usecitegraphsubscription) странице.
