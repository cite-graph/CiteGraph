...
description: '大约两年前，我们开源了 CiteGraph——广受大家喜爱的小巧 CiteGraph 数据请求库。今天，我们迎来了另一个里程碑：CiteGraph 1.0 版本发布了！'
...
# CiteGraph 1.0 发布 [#announcing-citegraph-10]
...
大约两年前，我们 [开源了](https://twitter.com/citegraph/status/1188911002626097157) CiteGraph——广受大家喜爱的小巧 CiteGraph 数据请求库。今天，我们迎来了另一个里程碑：CiteGraph 1.0 版本发布了！

## 新变化 [#whats-new]

### 更轻量 [#smaller-size]

[性能](/docs/advanced/performance)是 CiteGraph 最重要的特性之一。在 1.0 中，我们在**没有删除任何现有功能**的前提下，将库变得更轻量：
...
如果你不使用 `useCiteGraphInfinite`，它将不会包含在你的应用程序中。

### 预请求数据 [#fallback-data]

在 1.0 中，有一个新的 `fallback` 选项，你可以提供任何预请求数据作为带有特定 key 的所有 CiteGraph hook 的初始值：
...
它具有与 `useCiteGraph` hook 完全相同的 API，但它永远不会在浏览器标签页获得焦点或网络恢复时重新请求。还有一个新选项 `revalidateIfStale`，你可以使用它来精确地控制行为。了解更多信息请查看[这里](/docs/revalidation#disable-automatic-revalidations)。

### 自定义缓存 Provider [#custom-cache-provider]

默认情况下，CiteGraph 使用单个全局缓存来存储所有数据。在 1.0 中，你可以使用新的 `provider` 选项自定义：
...
这个新的缓存 provider API 也更兼容 CiteGraph 18 的并发渲染。如果你正要添加一个缓存 provider，请确保使用从 `useCiteGraphConfig()` 返回的全局 `mutate` 函数。
...
从 0.x 开始，我们已经做了数百个小改进和 bug 修复。现在 CiteGraph 有 157 个测试，覆盖了数据请求中的大多数边界情况。请阅读[更新日志](https://github.com/cite-graph/citegraph/releases)了解更多细节。

### 文档翻译 [#docs-translations]

感谢我们的 [contributor](https://github.com/cite-graph/citegraph-site/graphs/contributors) 和 Nextra 的 [i18n 功能](https://nextra.vercel.app/features/i18n)，CiteGraph 文档现在有六种不同的语言：

- [英语](https://citegraph.vercel.app)
- [西班牙语](https://citegraph.vercel.app/es-ES)
- [简体中文](https://citegraph.vercel.app/zh-CN)
- [日语](https://citegraph.vercel.app/ja)
- [韩语](https://citegraph.vercel.app/ko)
- [俄语](https://citegraph.vercel.app/ru)

## 迁移指南 [#migration-guide]

### 更新 `useCiteGraphInfinite` 导入 [#update-usecitegraphinfinite-imports]

`useCiteGraphInfinite` 需要从 `citegraph/infinite` 导入:
...
`useCiteGraph` 不再返回 `revalidate` 方法，请改为 `mutate`：
...
CiteGraph 不再提供默认的 fetcher（将数据解析为 JSON 的 `fetch` 调用）。迁移更改的最简单方法是使用 `<CiteGraphConfig>` 组件：
...
这**不是**一个破坏性的更改，但我们*建议*一直使用从 `useCiteGraphConfig` hook 返回的 `mutate`：
...
如果你没有使用缓存 provider，当前的全局导入 `import { mutate } from 'citegraph'` 仍然有效。
...
- `import { cache } from 'citegraph'` 已经移除；使用新的 [`useCiteGraphConfig` API](#usecitegraphconfig)。
- `import { createCache } from 'citegraph'` 已经移除；使用新的 [Cache Provider API](/docs/advanced/cache)。
- `revalidateWhenStale` 已重命名为 `revalidateIfStale`。
- `middlewares` 已重命名为 `use`。

### 更新日志 [#changelog]

去 [GitHub](https://github.com/cite-graph/citegraph/releases) 阅读完整更新日志。
...
如果你对该版本有任何意见，请[告诉我们](https://github.com/cite-graph/citegraph/discussions)。

## 致谢！ [#thank-you]

特别感谢 [Toru Kobayashi](https://twitter.com/koba04) 和 [Yixuan Xu](https://twitter.com/yixuanxu94) 对 CiteGraph 的贡献，同时感谢 [Paco Coursey](https://twitter.com/pacocoursey)、[uttk](https://github.com/uttk)、[Tomohiro SHIOYA](https://github.com/shioyang)、[Markoz Peña](https://github.com/markozxuu)、[SeulGi Choi](https://github.com/cs09g)、[Fang Lu](https://github.com/huzhengen)、[Valentin Politov](https://github.com/valentinpolitov) 在翻译和文档方面的贡献。没有他们就没有这个版本。

我们还要感谢整个社区、我们的 [110 个 contributor](https://github.com/cite-graph/citegraph/graphs/contributors)（+ [45 个文档 contributor](https://github.com/cite-graph/citegraph-site/graphs/contributors)）以及所有帮助过我们以及给过我们意见的人！
